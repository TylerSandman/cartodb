  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    _addLayerDialog: function(e) {
      ...
      if (this.options.user.featureEnabled('new_modals')) {
        ...
        // Fetch visible layer datasource options from the config if they are available.
        // Otherwise, use default enabled tabs.
        var defaultTabs = ['layer', 'file', 'gdrive', 'dropbox', 'twitter', 'scratch', 'arcgis', 'salesforce', 'success', 'error'];
        var enabledTabs = cdb.config.get('layer_tabs') || defaultTabs;

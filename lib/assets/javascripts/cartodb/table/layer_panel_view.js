  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    setDataLayer: function(dataLayer) {
      ...
      sections = self._getModuleSections();
      self.addModule(sql.render(),          sections['sql']);
      self.addModule(wizards.render(),      sections['wizards']);
      self.addModule(infowindow.render(),   sections['infoWindow']);
      self.addModule(editorPanel.render(),  sections['editorPanel']);
      self.addModule(legends.render(),      sections['legends']);
      self.addModule(this.filters.render(), sections['filters']);
    ...
    // Get the sections enabled for each module based
    // on config or defaults
    _getModuleSections(){
        var sections = {};

        // Defaults
        sections['sql'] = ['table', 'map', 'tableLite', 'mapLite'];
        sections['py'] = ['table', 'map', 'tableLite', 'mapLite'];
        sections['wizards'] = ['map', 'mapLite'];
        sections['infoWindow'] = ['map', 'mapLite'];
        sections['editorPanel'] = ['map', 'mapLite'];
        sections['legends'] = ['map', 'mapLite'];
        sections['filters'] = ['table', 'tableLite', 'map', 'mapLite'];

        // Global module configurations
        var modConfig = cdb.config.get('menu_modules');
        if (!modConfig) return sections;

        if (modConfig.indexOf('sql') === -1) sections['sql'] = [];
        if (modConfig.indexOf('py') === -1) sections['py'] = [];
        if (modConfig.indexOf('wizards') === -1) sections['wizards'] = [];
        if (modConfig.indexOf('infoWindow') === -1) sections['infoWindow'] = [];
        if (modConfig.indexOf('editorPanel') === -1) sections['editorPanel'] = [];
        if (modConfig.indexOf('legends') === -1) sections['legends'] = [];
        if (modConfig.indexOf('filters') === -1) sections['filters'] = [];
